...
async function migrateTeams() {
  ...
  const teamFiles = fs.readdirSync(teamsDir).filter(file => file.endsWith('.json'));
...
async function migratePlayersFromTeams() {
  ...
  for (const file of teamFiles) {
    const teamId = parseInt(path.basename(file, '.json'));
    const teamData = JSON.parse(fs.readFileSync(path.join(teamsDir, file), 'utf-8'));
    ...
    if (teamData.players && Array.isArray(teamData.players)) {
      ...
      console.log(`✓ Processed ${teamData.players.length} players for team ${teamId}`);
    }
  }
}
...
async function migratePlayerWeeks() {
  ...
  const playerFiles = fs.readdirSync(playersDir).filter(file => file.endsWith('.json'));

  for (const file of playerFiles) {
    const playerId = parseInt(path.basename(file, '.json'));
    const playerData = JSON.parse(fs.readFileSync(path.join(playersDir, file), 'utf-8'));
    ...
    if (playerData.weeks && Array.isArray(playerData.weeks)) {
      ...
      console.log(`✓ Processed ${playerData.weeks.length} weeks for player ${playerId}`);
    }
  }
}
...
async function migrateScoutedPlayers() {
  ...
  const scoutedFiles = fs.readdirSync(scoutedDir).filter(file => file.endsWith('.json'));

  for (const file of scoutedFiles) {
    const scoutedData = JSON.parse(fs.readFileSync(path.join(scoutedDir, file), 'utf-8'));
    ...
    if (playerError) {
      console.error(`Error upserting scouted player ${scoutedData.id}:`, playerError);
      continue;
    }
    ...
    if (scoutedData.scoutings && Array.isArray(scoutedData.scoutings)) {
      for (const scouting of scoutedData.scoutings) {
        ...
        if (scoutingError) {
          console.error(`Error inserting scouting for player ${scoutedData.id}:`, scoutingError);
        }
      }
      console.log(`✓ Processed ${scoutedData.scoutings.length} scoutings for player ${scoutedData.id}`);
    }
  }
}
